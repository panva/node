...
MaybeLocal<Value> ToWebCryptoJobResult(Environment* env, Local<Value> value) {
  ...
  if (value->IsArray()) {
    ...
    for (uint32_t n = 0; n < length; n++) {
      Local<Value> element;
      if (!array->Get(context, n).ToLocal(&element)) {
        return {};
      }
      ...
      if (element->IsArrayBuffer()) {
        ...
        converted = element.As<ArrayBufferView>()->Buffer();
      } else {
        CHECK(element->IsBoolean() || element->IsNullOrUndefined() ||
              element->IsNumber() || element->IsString());
        converted = element;
      }

      if (result->Set(context, n, converted).IsNothing()) {
        return {};
      }
    }
    return result;
  ...
  CHECK(value->IsBoolean() || value->IsNullOrUndefined() || value->IsNumber() ||
