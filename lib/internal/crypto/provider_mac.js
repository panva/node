...
function macOperation(name, key, options, data, oneShot) {
  let digest;
  let cipher;
  let iv;
  let customization;
  ...
  if (options !== undefined) {
    if (oneShot && typeof options === 'string') {
      outputEncoding = normalizeOutputEncoding(options);
    } else {
      ...
      if (oneShot) {
        outputEncoding = normalizeOutputEncoding(options.outputEncoding);
      }
    }
  }
  ...
  if (!hasExtendedOptions) {
    if (oneShot) {
      return oneShotMac(
        name, id, cache, key, digest, data, encodingsMap[outputEncoding]);
    }
...
function createHandle(algorithm, key, options) {
  return macOperation(
    validateName(algorithm, 'algorithm'), key, options, undefined, false);
}
...
function normalizeOutputEncoding(outputEncoding) {
  ...
  if (outputEncoding === 'buffer' || outputEncoding === 'hex') {
...
function mac(algorithm, key, data, options) {
  const name = validateName(algorithm, 'algorithm');
  if (typeof data !== 'string' && !isArrayBufferView(data)) {
    throw new ERR_INVALID_ARG_TYPE(
      'data', ['string', 'Buffer', 'TypedArray', 'DataView'], data);
  }
  return macOperation(name, key, options, data, true);
}
