...
async function eddsaSignVerify(key, data, algorithm, signature, prehashed) {
  ...
  // Ed25519ph (SHA-512) and Ed448ph (SHAKE256) both require a 64-byte digest
  if (prehashed && data.byteLength !== 64)
    throw lazyDOMException(
  ...
  return await jobPromise(() => new SignJob(
    ...
    undefined,
    undefined,
    prehashed || algorithm.name === 'Ed448' ?
      algorithm.context : undefined,
    signature,
    prehashed));
}
