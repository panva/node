...
function pubLen(key) {
  switch (key.algorithm.namedCurve) {
    case 'P-256': return 64;
    case 'P-384': return 96;
    case 'P-521': return 132;
  }
}
...
function ecExportKey(key, format) {
  try {
    if (format === 'pkcs8') {
      return key[kKeyObject][kHandle].export(kKeyFormatDER, kKeyEncodingPKCS8, null, null).buffer;
    }

    if (format === 'spki') {
      return key[kKeyObject][kHandle].export(kKeyFormatDER, kKeyEncodingSPKI).buffer;
    }

    // raw
    const len = pubLen(key);
    let { x, y } = key[kKeyObject][kHandle].exportJwk({}, false);
    x = Buffer.alloc(len / 2, x, 'base64url');
    y = Buffer.alloc(len / 2, y, 'base64url');
    const buf = Buffer.alloc(pubLen(key) + 1);
    buf[0] = 0x04;
    buf.set(x, 1);
    buf.set(y, 1 + len / 2);
    return buf.buffer;
