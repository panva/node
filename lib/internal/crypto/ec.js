...
async function ecdsaSignVerify(key, data, { name, hash }, signature, prehashed) {
  ...
  if (prehashed) {
    const expectedLength = getDigestSizeInBytes(hash.name);
    if (data.byteLength !== expectedLength)
      throw lazyDOMException(
        `Digest length must be ${expectedLength} bytes for ${hash.name}`,
        'OperationError');
  }
