...
function getOaepHash(options) {
  if (typeof options === 'object' && options.oaepHash !== undefined) {
    validateString(options.oaepHash, 'options.oaepHash');
    return options.oaepHash;
  }
  return undefined;
}
function getOaepLabel(options) {
  if (typeof options === 'object' && options.oaepLabel !== undefined) {
    return getArrayBufferView(options.oaepLabel, 'options.oaepLabel');
  }
  return undefined;
}

function getOutputLength(options) {
  let outputLength = getIntOption('outputLength', options);
...
async function publicEncrypt(data, key, options) {
  data = getData(data);
  ...
  const oaepHash = getOaepHash(options);
  const oaepLabel = getOaepLabel(options);
  const padding = getPadding(options);
}

async function privateDecrypt(data, key, options) {
  data = getData(data);
