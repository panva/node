...
/**
 * Truncates a Uint8Array to ensure it only contains the specified number of bits.
 * @param {number} lengthBits
 * @param {Uint8Array} bytes
 * @returns {Uint8Array}
 */
function truncateToBitLength(lengthBits, bytes) {
  const lengthBytes = MathCeil(lengthBits / 8);

  if (bytes.length !== lengthBytes) {
    if (lengthBytes >= bytes.length) {
      throw new ERR_OUT_OF_RANGE('lengthBits', '< bytes.length', lengthBits);
    }
    bytes = TypedArrayPrototypeSlice(bytes, 0, lengthBytes);
  }

  const remainderBits = lengthBits % 8;

  // Zero any "unused bits" in the final byte.
  if (remainderBits) {
    bytes[bytes.length - 1] &= ~((0xFF) >> remainderBits);
  }
