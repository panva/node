...
const conditionalAlgorithms = {
  ...
  'MLKEM768-P256': !isFips && !!EVP_PKEY_ML_KEM_768 &&
    (!process.features.openssl_is_boringssl ||
      (ArrayPrototypeIncludes(getHashes(), 'sha3-256') &&
       ArrayPrototypeIncludes(getHashes(), 'shake256'))),
  'MLKEM768-X25519': !isFips && !!EVP_PKEY_ML_KEM_768 &&
    (!process.features.openssl_is_boringssl ||
      (ArrayPrototypeIncludes(getHashes(), 'sha3-256') &&
       ArrayPrototypeIncludes(getHashes(), 'shake256'))),
  'MLKEM1024-P384': !isFips && !!EVP_PKEY_ML_KEM_1024 &&
    (!process.features.openssl_is_boringssl ||
      (ArrayPrototypeIncludes(getHashes(), 'sha3-256') &&
       ArrayPrototypeIncludes(getHashes(), 'shake256'))),
...
// Experimental algorithms that emit a warning. Some experimental algorithms do
// not emit a warning because they are deemed stable enough given their browser
// adoption.
const experimentalAlgorithmsWithWarning = [
  'AES-OCB',
  'Argon2d',
  'Argon2i',
  'Argon2id',
...
function createSupportedAlgorithms(algorithmDefs) {
  ...
  for (let i = 0; i < algorithmEntries.length; i++) {
    ...
    for (let j = 0; j < operationEntries.length; j++) {
      ...
      // Add experimental warnings for algorithms that require them
      if (ArrayPrototypeIncludes(experimentalAlgorithmsWithWarning, algorithmName)) {
