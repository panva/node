...
async function rsaSignVerify(key, data, { saltLength }, signature, prehashed) {
  ...
  const hashName = key[kAlgorithm].hash.name;
  const digestLength = getDigestSizeInBytes(hashName);

  if (prehashed && data.byteLength !== digestLength)
    throw lazyDOMException(
      `Digest length must be ${digestLength} bytes for ${hashName}`,
      'OperationError');
  ...
  return await jobPromise(() => {
    if (key[kAlgorithm].name === 'RSA-PSS') {
      validateInt32(
        ...
        0,
        MathCeil((key[kAlgorithm].modulusLength - 1) / 8) - digestLength - 2);
    }
