...
async function exportKeyJWK(key) {
  ...
  switch (key.algorithm.name) {
    ...
    case 'AES-KW':
      // Fall through
    case 'AES-OCB': {
      const alg = require('internal/crypto/aes')
...
function supports(
  operation,
  algorithm,
  additionalAlgorithm = null,
) {
  emitExperimentalWarning('The SubtleCrypto.prototype.supports method');
  if (this !== subtle) throw new ERR_INVALID_THIS('SubtleCrypto');
  ...
  algorithm = webidl.converters.AlgorithmIdentifier(algorithm, {
    prefix,
    context: '2rd argument',
  });

  // TODO(@panva): look at supports(operation, algorithm, length?) more thoroughly
  additionalAlgorithm = webidl.converters.AlgorithmIdentifier(additionalAlgorithm, {
    prefix,
    context: '3rd argument',
  });

  if (!ArrayPrototypeIncludes(SUPPORTED_OPERATIONS, operation)) {
    return false;
  }
  ...
  switch (operation) {
    case 'deriveKey':
      if (
        !checkAlgorithmSupport('importKey', additionalAlgorithm) ||
        !checkAlgorithmSupport('get key length', additionalAlgorithm)
      ) {
        return false;
      }
      break;
    case 'wrapKey':
      if (
        !checkAlgorithmSupport('exportKey', additionalAlgorithm)
      ) {
        return false;
      }
      break;
  ...
  return checkAlgorithmSupport(operation, algorithm);
}

function checkAlgorithmSupport(op, alg) {
  if (op === 'deriveKey')
    op = 'deriveBits';
  ...
  try {
    ...
    if (op === 'wrapKey') {
      return checkAlgorithmSupport(alg, 'encrypt');
    } else if (op === 'unwrapKey') {
      return checkAlgorithmSupport(alg, 'decrypt');
    }
    return false;
  }

  // The only thing for which we recognize an AlgorithmIdentifier but
  // don't support the use of its context option
  if (
    normalizedAlgorithm.name === 'Ed448' &&
    (op === 'sign' || op === 'verify') &&
    normalizedAlgorithm.context?.byteLength
  ) {
    return false;
  }

  // TODO(@panva): how to deal with additional validation of the alg
  // properties?
  // e.g. allowed AeadParams.tagLength values
