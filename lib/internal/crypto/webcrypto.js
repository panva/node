...
function deriveBits(algorithm, baseKey, length = null) {
  try {
    ...
    switch (algorithm.name) {
      ...
      case 'HKDF': {
        const bits = require('internal/crypto/hkdf')
          .hkdfDeriveBits(algorithm, baseKey, length);
        return length === 0 ? PromiseResolve(bits) : bits;
      }
      case 'PBKDF2': {
        const bits = require('internal/crypto/pbkdf2')
...
function getPublicKey(key, keyUsages) {
  try {
    ...
    return PromiseResolve(keyObject.toCryptoKey(getCryptoKeyAlgorithm(key), true, keyUsages));
  } catch (err) {
    return PromiseReject(err);
  }
}
...
function encapsulateKey(encapsulationAlgorithm, encapsulationKey, sharedKeyAlgorithm, extractable, usages) {
  try {
