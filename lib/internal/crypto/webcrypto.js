...
async function generateKey(
  ...
  switch (algorithm.name) {
    ...
    case 'MLKEM768-X25519':
      resultType = 'CryptoKeyPair';
      result = await require('internal/crypto/mlkem_x25519')
        .mlkemX25519GenerateKey(algorithm, extractable, keyUsages);
      break;
    case 'KMAC128':
      // Fall through
    case 'KMAC256':
...
async function exportKeyRawPublic(key, format) {
  switch (getCryptoKeyAlgorithm(key).name) {
    ...
    case 'MLKEM768-X25519': {
      ...
      return require('internal/crypto/mlkem_x25519')
        .mlkemX25519ExportKey(key, format);
    }
    default:
      return undefined;
  }
...
async function exportKeyRawSeed(key) {
  switch (getCryptoKeyAlgorithm(key).name) {
    ...
    case 'MLKEM768-X25519':
      return require('internal/crypto/mlkem_x25519')
        .mlkemX25519ExportKey(key, 'raw-seed');
    default:
      return undefined;
  }
}
...
function importKeySync(format, keyData, algorithm, extractable, keyUsages) {
  ...
  switch (algorithm.name) {
    ...
    case 'ML-KEM-1024':
      ...
      break;
    case 'MLKEM768-X25519':
      result = require('internal/crypto/mlkem_x25519')
        .mlkemX25519ImportKey(format, keyData, algorithm, extractable, keyUsages);
      break;
  }
...
async function getPublicKey(key, keyUsages) {
  ...
  if (getCryptoKeyAlgorithm(key).name === 'MLKEM768-X25519') {
    return require('internal/crypto/mlkem_x25519')
      .mlkemX25519GetPublicKey(key, keyUsages);
  }
...
async function encapsulateBits(encapsulationAlgorithm, encapsulationKey) {
  ...
  switch (keyAlgorithm.name) {
    ...
    case 'MLKEM768-X25519':
      return await require('internal/crypto/mlkem_x25519')
        .mlkemX25519Encapsulate(encapsulationKey);
  }
...
async function encapsulateKey(encapsulationAlgorithm, encapsulationKey, sharedKeyAlgorithm, extractable, usages) {
  ...
  switch (keyAlgorithm.name) {
    ...
    case 'ML-KEM-1024':
      ...
      break;
    case 'MLKEM768-X25519':
      encapsulateBits = await require('internal/crypto/mlkem_x25519')
        .mlkemX25519Encapsulate(encapsulationKey);
      break;
    default:
      throw lazyDOMException('Unrecognized algorithm name', 'NotSupportedError');
...
async function decapsulateBits(decapsulationAlgorithm, decapsulationKey, ciphertext) {
  ...
  switch (keyAlgorithm.name) {
    ...
    case 'MLKEM768-X25519':
      return await require('internal/crypto/mlkem_x25519')
        .mlkemX25519Decapsulate(decapsulationKey, ciphertext);
  }
...
async function decapsulateKey(
  ...
  switch (keyAlgorithm.name) {
    ...
    case 'ML-KEM-1024':
      ...
      break;
    case 'MLKEM768-X25519':
      decapsulatedBits = await require('internal/crypto/mlkem_x25519')
        .mlkemX25519Decapsulate(decapsulationKey, ciphertext);
      break;
    default:
      throw lazyDOMException('Unrecognized algorithm name', 'NotSupportedError');
