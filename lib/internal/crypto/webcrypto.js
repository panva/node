...
async function generateKey(
  ...
  switch (algorithm.name) {
    ...
    case 'SLH-DSA-SHAKE-256s':
      resultType = 'CryptoKeyPair';
      result = await require('internal/crypto/slh_dsa')
        .slhDsaGenerateKey(algorithm, extractable, keyUsages);
      break;
    case 'ML-KEM-512':
      // Fall through
    case 'ML-KEM-768':
...
async function exportKeySpki(key) {
  switch (key[kAlgorithm].name) {
    ...
    case 'SLH-DSA-SHAKE-256f':
      // Fall through
    case 'SLH-DSA-SHAKE-256s':
      return require('internal/crypto/slh_dsa')
        .slhDsaExportKey(key, kWebCryptoKeyFormatSPKI);
    case 'ML-KEM-512':
      // Fall through
    case 'ML-KEM-768':
      // Fall through
...
async function exportKeyPkcs8(key) {
  switch (key[kAlgorithm].name) {
    ...
    case 'SLH-DSA-SHAKE-256s':
      return require('internal/crypto/slh_dsa')
        .slhDsaExportKey(key, kWebCryptoKeyFormatPKCS8);
    case 'ML-KEM-512':
      // Fall through
    case 'ML-KEM-768':
      // Fall through
...
async function exportKeyRawPublic(key, format) {
  switch (key[kAlgorithm].name) {
    ...
    case 'SLH-DSA-SHAKE-256f':
      // Fall through
    case 'SLH-DSA-SHAKE-256s': {
      // SLH-DSA keys don't recognize "raw"
      if (format !== 'raw-public') {
        return undefined;
      }
      return require('internal/crypto/slh_dsa')
        .slhDsaExportKey(key, kWebCryptoKeyFormatRaw);
    }
    default:
      return undefined;
  }
...
async function exportKeyRawPrivate(key) {
  switch (key[kAlgorithm].name) {
    case 'SLH-DSA-SHA2-128f':
      // Fall through
    case 'SLH-DSA-SHA2-128s':
...
async function exportKey(format, key) {
  ...
  switch (format) {
    ...
    case 'raw-private': {
      if (key[kKeyType] === 'private') {
        result = await exportKeyRawPrivate(key);
      }
      break;
    }
    case 'raw': {
...
function importKeySync(format, keyData, algorithm, extractable, keyUsages) {
  ...
  switch (algorithm.name) {
    ...
    case 'SLH-DSA-SHAKE-256f':
      // Fall through
    case 'SLH-DSA-SHAKE-256s':
      result = require('internal/crypto/slh_dsa')
        .slhDsaImportKey(format, keyData, algorithm, extractable, keyUsages);
      break;
    case 'ML-KEM-512':
      // Fall through
    case 'ML-KEM-768':
...
async function signVerify(algorithm, key, data, signature) {
  ...
  switch (algorithm.name) {
    ...
    case 'SLH-DSA-SHAKE-256f':
      // Fall through
    case 'SLH-DSA-SHAKE-256s':
      return await require('internal/crypto/slh_dsa')
        .slhDsaSignVerify(key, data, algorithm, signature);
    case 'KMAC128':
      // Fall through
    case 'KMAC256':
      return await require('internal/crypto/mac')
