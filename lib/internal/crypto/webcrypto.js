...
async function signDigestVerifyDigest(algorithm, key, digest, signature) {
  let usage = 'sign';
  let op = 'signDigest';
  if (signature !== undefined) {
    usage = 'verify';
  ...
  switch (algorithm.name) {
    ...
    case 'RSASSA-PKCS1-v1_5':
      return await require('internal/crypto/rsa')
        .rsaSignVerify(key, digest, algorithm, signature, true);
    case 'ECDSA':
      return await require('internal/crypto/ec')
        .ecdsaSignVerify(key, digest, algorithm, signature, true);
    case 'Ed25519':
      // Fall through
    case 'Ed448':
      return await require('internal/crypto/cfrg')
        .eddsaSignVerify(key, digest, algorithm, signature, true);
    case 'ML-DSA-44':
      // Fall through
    case 'ML-DSA-65':
      // Fall through
    case 'ML-DSA-87':
      return await require('internal/crypto/ml_dsa')
        .mlDsaSignVerify(key, digest, algorithm, signature, true);
  }
  throw lazyDOMException('Unrecognized algorithm name', 'NotSupportedError');
}
...
async function signDigest(algorithm, key, digest) {
  if (this !== subtle) throw new ERR_INVALID_THIS('SubtleCrypto');

  webidl ??= require('internal/crypto/webidl');
  const prefix = "Failed to execute 'signDigest' on 'SubtleCrypto'";
  webidl.requiredArguments(arguments.length, 3, { prefix });
  algorithm = webidl.converters.AlgorithmIdentifier(algorithm, {
    prefix,
    context: '1st argument',
  ...
  digest = webidl.converters.BufferSource(digest, {
    prefix,
    context: '3rd argument',
  });

  return await signDigestVerifyDigest(algorithm, key, digest);
}

async function verifyDigest(algorithm, key, signature, digest) {
  if (this !== subtle) throw new ERR_INVALID_THIS('SubtleCrypto');

  webidl ??= require('internal/crypto/webidl');
  const prefix = "Failed to execute 'verifyDigest' on 'SubtleCrypto'";
  webidl.requiredArguments(arguments.length, 4, { prefix });
  algorithm = webidl.converters.AlgorithmIdentifier(algorithm, {
    prefix,
    context: '1st argument',
  ...
  return await signDigestVerifyDigest(algorithm, key, digest, signature);
}
