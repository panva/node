...
async function encapsulateBits(encapsulationAlgorithm, encapsulationKey) {
  ...
  if (normalizedEncapsulationAlgorithm.name !== encapsulationKey.algorithm.name) {
    throw lazyDOMException(
...
async function encapsulateKey(encapsulationAlgorithm, encapsulationKey, sharedKeyAlgorithm, extractable, usages) {
  ...
  let encapsulateBits; // TODO:

  const sharedKey = await ReflectApply(
    importKey,
    this,
    ['raw-secret', encapsulateBits.sharedKey, sharedKeyAlgorithm, extractable, usages],
  );
...
async function decapsulateBits(decapsulationAlgorithm, decapsulationKey, ciphertext) {
  ...
  if (normalizedDecapsulationAlgorithm.name !== decapsulationKey.algorithm.name) {
    throw lazyDOMException(
      'key algorithm mismatch',
      'InvalidAccessError');
  }
  ...
  let decapsulatedBits; // TODO:

  return decapsulatedBits;
}
...
async function decapsulateKey(
  ...
  return ReflectApply(
    importKey,
    this,
    ['raw-secret', decapsulatedBits, sharedKeyAlgorithm, extractable, usages],
  );
}
