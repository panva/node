...
const {
  CShakeJob,
  hasAlgorithm,
  kAlgorithmTypeAsymmetricCipher,
  kAlgorithmTypeCipher,
  kAlgorithmTypeDigest,
  kAlgorithmTypeKdf,
  kAlgorithmTypeKem,
  kAlgorithmTypeKeyExchange,
  kAlgorithmTypeKeyManagement,
  kAlgorithmTypeMac,
  kAlgorithmTypeSignature,
...
function check(op, alg, length) {
  ...
  switch (op) {
    case 'digest': {
      // A customized cSHAKE is computed from the KECCAK-KMAC primitive, which
      // Node.js is not always built with.
      if (isCustomizedCShake(normalizedAlgorithm) && CShakeJob === undefined) {
        return false;
      }
  ...
  return isBackendOperationAvailable(op, normalizedAlgorithm);
}

// OpenSSL fetches AES ciphers under their full name, which embeds the key
// length. Web Crypto AES algorithm identifiers are key length agnostic, so a
// mode counts as available when any AES key length implements it.
function hasAesCipherMode(mode) {
  return hasAlgorithm(kAlgorithmTypeCipher, `AES-128-${mode}`) ||
         hasAlgorithm(kAlgorithmTypeCipher, `AES-192-${mode}`) ||
         hasAlgorithm(kAlgorithmTypeCipher, `AES-256-${mode}`);
}

// A function name or customization string turns cSHAKE into KECCAK-KMAC, plain
// SHAKE is used otherwise. KT128 and KT256 also accept a customization, hence
// the name check.
function isCustomizedCShake({ name, functionName, customization }) {
  return (name === 'cSHAKE128' || name === 'cSHAKE256') &&
         !!(functionName?.byteLength || customization?.byteLength);
}

// A zero length KMAC output needs no backend algorithm at all. OpenSSL's KMAC
// only produces whole bytes, other bit lengths are computed from the underlying
// KECCAK-KMAC digest, which is also the fallback when KMAC itself is missing.
// outputLength is a required KmacParams member, so it is always present here.
function hasKmac(name, outputLength) {
  if (outputLength === 0) {
    return true;
  }

  if (outputLength % 8 === 0 && hasAlgorithm(kAlgorithmTypeMac, name)) {
    return true;
  }

  return hasAlgorithm(kAlgorithmTypeDigest,
                      `KECCAK-KMAC-${StringPrototypeSlice(name, 4)}`);
}

// Determines whether the active OpenSSL providers implement the algorithms the
// operation is built from. The names below are OpenSSL algorithm names; lookups
// are case-insensitive and the Web Crypto names are registered aliases, so most
// of them are passed through as-is. `op` and `normalizedAlgorithm.name` have
// already been constrained to a registered pair by normalizeAlgorithm, and the
// hash to a SHA digest by the WebIDL converters, so those are not rechecked.
//
// This answers a question only providers can answer, which is why it runs per
// call rather than being cached: the answer changes with the FIPS state. What
// the build supports at all is a separate, startup-fixed question answered by
// conditionalAlgorithms in internal/crypto/util, which drops unavailable
// algorithms from the registry so normalizeAlgorithm rejects them before this
// is reached. On BoringSSL and OpenSSL < 3 there are no providers to query,
// hasAlgorithm() always reports true, and that registry is the only gate.
function isBackendOperationAvailable(op, normalizedAlgorithm) {
  const { name } = normalizedAlgorithm;
  ...
  switch (op) {
    case 'digest':
      switch (name) {
        ...
        case 'TurboSHAKE128':
        case 'TurboSHAKE256':
          // Implemented in Node.js, not fetched from a provider.
          return true;
        case 'cSHAKE128':
        case 'cSHAKE256': {
          const bits = StringPrototypeSlice(name, 6);
          return hasAlgorithm(
            kAlgorithmTypeDigest,
            isCustomizedCShake(normalizedAlgorithm) ?
              `KECCAK-KMAC-${bits}` :
              `SHAKE-${bits}`);
        }
        default:
          // SHA-1, SHA-256, SHA-384, SHA-512, SHA3-256, SHA3-384, SHA3-512.
          return hasAlgorithm(kAlgorithmTypeDigest, name);
      }
    case 'decrypt':
    case 'encrypt':
      switch (name) {
        ...
        case 'AES-GCM':
        case 'AES-OCB':
          // The Web Crypto name suffix is the OpenSSL cipher mode.
          return hasAesCipherMode(StringPrototypeSlice(name, 4));
        case 'RSA-OAEP':
          return hasAlgorithm(kAlgorithmTypeAsymmetricCipher, 'RSA');
        default:
          // ChaCha20-Poly1305.
          return hasAlgorithm(kAlgorithmTypeCipher, name);
      }
    case 'unwrapKey':
    case 'wrapKey':
      // AES-KW is the only algorithm registered for these operations.
      return hasAesCipherMode('WRAP');
    case 'sign':
    case 'verify':
      switch (name) {
        case 'ECDSA':
          return hasAlgorithm(kAlgorithmTypeSignature, name) &&
                 hasAlgorithm(kAlgorithmTypeDigest,
                              normalizedAlgorithm.hash.name);
        case 'HMAC':
          return hasAlgorithm(kAlgorithmTypeMac, name);
        case 'KMAC128':
        case 'KMAC256':
          return hasKmac(name, normalizedAlgorithm.outputLength);
        case 'RSA-PSS':
        case 'RSASSA-PKCS1-v1_5':
          return hasAlgorithm(kAlgorithmTypeSignature, 'RSA');
        default:
    ...
    case 'deriveBits':
      switch (name) {
        case 'HKDF':
          return hasAlgorithm(kAlgorithmTypeKdf, name) &&
                 hasAlgorithm(kAlgorithmTypeDigest,
                              normalizedAlgorithm.hash.name) &&
                 hasAlgorithm(kAlgorithmTypeMac, 'HMAC');
        case 'PBKDF2':
          return hasAlgorithm(kAlgorithmTypeKdf, name) &&
                 hasAlgorithm(kAlgorithmTypeDigest,
                              normalizedAlgorithm.hash.name);
        ...
        case 'X25519':
        case 'X448':
          return hasAlgorithm(kAlgorithmTypeKeyExchange, name);
        default:
    ...
    case 'encapsulate':
      ...
      return hasAlgorithm(kAlgorithmTypeKem, name);
    case 'generateKey':
      switch (name) {
        case 'AES-CBC':
        case 'AES-CTR':
        ...
        case 'KMAC128':
        case 'KMAC256':
          // Secret keys are generated from random bytes.
          return true;
        case 'ECDH':
        case 'ECDSA':
          return hasAlgorithm(kAlgorithmTypeKeyManagement, 'EC');
        case 'RSA-OAEP':
        case 'RSA-PSS':
        case 'RSASSA-PKCS1-v1_5':
          return hasAlgorithm(kAlgorithmTypeKeyManagement, 'RSA');
        default:
          // Ed25519, Ed448, X25519, X448, ML-DSA-*, ML-KEM-*.
          return hasAlgorithm(kAlgorithmTypeKeyManagement, name);
      }
    /* c8 ignore start */
    default: {
      const assert = require('internal/assert');
