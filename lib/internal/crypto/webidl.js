...
function validateCShakeCustomizationParameter(V, dict) {
  if (getBufferSourceByteLength(V) === 0) return;
  if (!ArrayPrototypeIncludes(getHashes(), StringPrototypeToLowerCase(dict.name)))
    throw lazyDOMException(
      'Unsupported CShakeParams customization',
      'NotSupportedError');
  validateCShakeCustomization(V, 'CShakeParams');
}
  ...
  'CShakeParams', [
    ...
    [
      ...
      },
      {
        key: 'customization',
        converter: converters.BufferSource,
        validator: validateCShakeCustomizationParameter,
      },
    ],
  ]);
    ...
    [
      {
        ...
        converter: (V, opts) =>
          converters['unsigned long'](V, enforceRangeOptions(opts)),
        validator: (V, dict) => {
          if ((V % 8 !== 0 && !hasKmacBitLengthSupport(dict.name)) || (V === 0 && isFips()))
            throw lazyDOMException(
      ...
      },
      {
        ...
        converter: converters.BufferSource,
        validator: (V, dict) => {
          if (dict.outputLength % 8 !== 0)
            validateCShakeCustomization(V, 'KmacParams');
        },
      },
    ],
  ]);
