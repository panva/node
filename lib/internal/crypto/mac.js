...
function normalizeKeyLength(handle, algorithm) {
  ...
  if (algorithm.name === 'KMAC128' || algorithm.name === 'KMAC256')
    validateKmacKeyLength(length, algorithm);
...
function kmacSignVerify(key, data, algorithm, signature) {
  ...
  const { length } = getCryptoKeyAlgorithm(key);
  if (length % 8 !== 0 && algorithm.customization !== undefined)
    validateCShakeCustomization(algorithm.customization, 'KmacParams');
  const mode = signature === undefined ? kSignJobModeSign : kSignJobModeVerify;
