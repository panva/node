...
const kGcmTagLengths = [32, 64, 96, 104, 112, 120, 128];
const kOcbTagLenghts = [64, 96, 128];
...
function asyncAesGcmCipher(
  ...
  if (!ArrayPrototypeIncludes(kGcmTagLengths, tagLength)) {
  ...
  switch (mode) {
    case kWebCryptoCipherDecrypt: {
      ...
      //
      // > If *ciphertext* has a length less than *tagLength* bits, then `throw`
      // > an `OperationError`.
...
function asyncAesOcbCipher(
  ...
  data,
  { iv, additionalData, tagLength = 128 }) {
  if (!ArrayPrototypeIncludes(kOcbTagLenghts, tagLength)) {
    return PromiseReject(lazyDOMException(
