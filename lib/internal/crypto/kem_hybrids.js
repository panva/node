...
// https://www.ietf.org/archive/id/draft-irtf-cfrg-concrete-hybrid-kems-03.html#section-4
// CG framework used by those instances:
// https://www.ietf.org/archive/id/draft-irtf-cfrg-hybrid-kems-10.html#section-5.5
...
const kKemPqPublicUsagesMask =
  getUsagesMask(new SafeSet(['encapsulateBits']));
const kKemPqPrivateUsagesMask =
  getUsagesMask(new SafeSet(['decapsulateBits']));
const kGroupPrivateUsagesMask =
...
/**
 * Creates a Uint8Array view over supported BufferSource-like data without
 * copying.
 * @param {ArrayBuffer | ArrayBufferView} data
 * @returns {Uint8Array}
 */
function bytesFromSource(data) {
...
function copyBytes(data) {
  return new Uint8Array(bytesFromSource(data));
}
...
/**
 * Creates the WebCrypto algorithm object for the PQ KEM component.
 * @param {object} config
 * @returns {{ name: string }}
 */
function kemPqAlgorithm(config) {
  return { name: config.kemPqName };
}

/**
 * Creates the WebCrypto algorithm object for the traditional group component.
 * @param {object} config
 * @returns {object}
 */
function groupAlgorithm(config) {
...
/**
 * Wraps a PQ KEM handle in an internal CryptoKey.
 * @param {KeyObjectHandle} handle
 * @param {'public' | 'private'} type
 * @param {object} config
 * @returns {InternalCryptoKey}
 */
function createKemPqCryptoKey(handle, type, config) {
  return new InternalCryptoKey(
    handle,
    kemPqAlgorithm(config),
    type === 'public' ? kKemPqPublicUsagesMask : kKemPqPrivateUsagesMask,
    type === 'public');
}

/**
 * Wraps a traditional group handle in an internal CryptoKey.
 * @param {KeyObjectHandle} handle
 * @param {'public' | 'private'} type
 * @param {object} config
 * @returns {InternalCryptoKey}
 */
function createGroupCryptoKey(handle, type, config) {
  return new InternalCryptoKey(
    handle,
    groupAlgorithm(config),
    type === 'private' ? kGroupPrivateUsagesMask : 0,
    type === 'public');
}
 ...
 * @param {InternalCryptoKey} publicKey
 * @param {InternalCryptoKey} privateKey
 * @param {object} config
 * @returns {Promise<Uint8Array>}
 */
function deriveGroupBits(publicKey, privateKey, config) {
  const { name } = groupAlgorithm(config);
  let bits;
  try {
    bits = ecdhDeriveBits({
      name,
...
function combineSharedSecret(
  ...
  // https://www.ietf.org/archive/id/draft-irtf-cfrg-hybrid-kems-10.html#section-5.1.3
...
function prepareEncapsulationGroup(kemPqHandle, groupHandle, config) {
  ...
  const sharedSecretT = deriveGroupBits(
    createGroupCryptoKey(groupHandle, 'public', config),
    createGroupCryptoKey(ephemeralGroupHandle, 'private', config),
    config);
  return {
    __proto__: null,
    ciphertextT,
...
function kemHybridEncaps(encapsulationKey) {
  ...
  const encapsPq = mlKemEncapsulate(
    createKemPqCryptoKey(kemPqHandle, 'public', config));
  const {
    promise,
    resolve,
    reject,
...
function kemHybridDecaps(decapsulationKey, ciphertext) {
  ...
  const sharedSecretPq = mlKemDecapsulate(
    createKemPqCryptoKey(kemPqHandle, 'private', config),
    ciphertextPq);
  const {
    promise,
    resolve,
  ...
  continueJob(sharedSecretPq, reject, (sharedSecretPq) => {
    const sharedSecretT = deriveGroupBits(
      createGroupCryptoKey(groupPublicHandle, 'public', config),
      createGroupCryptoKey(groupHandle, 'private', config),
      config);
