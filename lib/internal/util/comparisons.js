...
function objectComparisonStart(val1, val2, mode, memos) {
  ...
  if (val1[SymbolToStringTag] === undefined &&
    ...
    const val1KeyData = getCryptoKeyData(val1);
    const val2KeyData = isCryptoKey(val2) ?
      getCryptoKeyData(val2) : undefined;
    if (!isCryptoKey(val2) ||
      ...
      (val1KeyData === undefined) !== (val2KeyData === undefined) ||
      (val1KeyData !== undefined &&
        !innerDeepEqual(val1KeyData, val2KeyData, mode, memos))
    ) {
      return false;
    }
