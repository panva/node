...
The `functionName` member represents the NIST function-name string used to
...
* the UTF-8 encoding of `'KMAC'`[^openssl40]
* the UTF-8 encoding of `'TupleHash'`[^openssl40]
* the UTF-8 encoding of `'ParallelHash'`[^openssl40]
...
The `customization` member represents the customization string. Accepted
values are:
